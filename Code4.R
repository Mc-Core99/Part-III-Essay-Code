...
relaxed.minimum <- function(AMat, relaxed.objective.function, b1, relaxed.objective.derivative) {
  relaxed.solution <<- spg(par = rep(0,2*length(hyperplanes[1, 1, ])*length(X[1, ])), 
                           ...
                           control = list(ftol = 1e-6, trace = TRUE, checkGrad = FALSE))
}
...
c <- 10
lambda <- 0.3
ratio.temp <- matrix(data = NA, nrow= 3, ncol = 7)
...
for (q in 1:3) {
  ...
  #for a quicker plot, make the end point 100
  #you will also need to change to 100 for the
  ...
  for (j in seq(30, 60, by =5)) {
    ...
    mini <- c()
    for (o in 1:3) {
      true.minimum(true.objective, epochs = 350, learning.rate = 0.05)
      mini[o] <- true.sol
    }
    true.sol <- min(mini)
...
ggplot(df, aes(x = seq(30, 60, by =5), y = ratio, color = "Ratio")) +
  geom_point() +
  geom_smooth(aes(color = "Ratio"), se = FALSE) +
  geom_errorbar(aes(ymin = ymin, ymax = ymax, color = "Errors")) +
  geom_line(data = baseline, mapping = aes(x = seq(25, 65, by =0.1), y = 0.5*baseline, color = "Shape-of-Upper-Bound")) +
  geom_hline(yintercept = 1, linetype = "dashed", color = "purple") +
  labs(
    x = "n",
    y = "Ratio",
    title = "Comparison of Experimental & Theoretical Results For Fixed Design"
  ) +
