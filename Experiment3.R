...
for (j in seq(50, 130, by =10)) {
  ...
  n <- j
  p <- ceiling(j/c)
  X <- matrix(rgamma(n*p,shape = 2, rate = 1.2), n, p)
  y <- matrix(rnorm(n, mean = 1/sqrt(n), sd = 1/sqrt(n)), n, 1)
...
#outlier for unifrom at n = 130 which we remove for the purposes
#of showing the conclusions better

ratio2 <- ratio2[1:8, 1]
ratio2 <- as.data.frame(ratio2)

Legend <- c("Log-Normal" = "blue", "Uniform" = "green", "Gamma" = "orange", "Shape-of-Upper-Bound" = "red")

ggplot(ratio1, aes(x = seq(50, 130, by = 10), y = ratio1, color = "Log-Normal")) +
  geom_smooth(se = FALSE) +
  geom_smooth(data = ratio2, mapping = aes(x = seq(50, 120, by = 10), y = ratio2, color = "Uniform"), se = FALSE) +
  geom_smooth(data = ratio3, mapping = aes(x = seq(50, 130, by = 10), y = ratio3, color = "Gamma"), se = FALSE) +
...
ggplot(ratio1, aes(x = seq(50, 130, by = 10), y = ratio1, color = "Log-Normal")) +
  geom_line() +
  geom_line(data = ratio2, mapping = aes(x = seq(50, 130, by = 10), y = ratio2, color = "Uniform")) +
  geom_line(data = ratio3, mapping = aes(x = seq(50, 130, by = 10), y = ratio3, color = "Gamma")) +
  geom_line(data = baseline, mapping = aes(x = seq(50, 130, by =  1), y = 0.03*baseline, color = "Shape-of-Upper-Bound")) +
  geom_hline(yintercept = exp(1), linetype = "dashed", color = "purple") +
  labs(
    x = "n",
