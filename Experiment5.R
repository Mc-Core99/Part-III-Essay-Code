...
ggplot(ratio, aes(x = seq(50, 200, by = 10), y = ratio, color = "Exp(ratio^2)")) +
  geom_smooth() +
  geom_line(data = baseline, mapping = aes(x = seq(50, 200, by =  1), y = 0.01*baseline, color = "Shape-of-Upper-Bound")) +
  geom_hline(yintercept = exp(1), linetype = "dashed", color = "purple") +
  labs(
    x = "n",
    y = "Exponential of Ratio Squared",
    title = "Comparison of Experimental & Theoretical Results In The Fixed Design Setting"
  ) +
