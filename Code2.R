...
baseline <- sqrt(log(2*seq(25, 65, by =0.1)))
baseline <- as.data.frame(baseline)

#plotting the results

Legend <- c("Increasing" = "blue", "Decreasing" = "green", "Shape-of-Upper-Bound" = "red", "Errors" = "black")
...
names(df) <- c('Decreasing', 'Increasing', 'ymax1', 'ymin1', 'ymax2', 'ymin2')
...
ggplot(df, aes(x = seq(30, 60, by = 5))) +
  geom_point(aes(y = Increasing, color = "Increasing")) +
  geom_errorbar(aes(ymin = ymin1, ymax = ymax1, color = "Errors")) +
  geom_smooth(aes(y = Increasing, color = "Increasing"), se = FALSE) +
  geom_point(aes(y = Decreasing, color = "Decreasing")) +
  geom_errorbar(aes(ymin = ymin2, ymax = ymax2, color = "Errors")) +
  geom_smooth(aes(y = Decreasing, color = "Decreasing"), se = FALSE) +
  geom_line(data = baseline, aes(x = seq(25, 65, by = 0.1), y = baseline, color = "Shape-of-Upper-Bound")) +
  geom_hline(yintercept = 1, linetype = "dashed", color = "purple") +
  labs(
    x = "n",
    y = "Ratio",
    title = "Comparison of Experimental & Theoretical Results For Non Constant n/p",
    color = "Legend"
  ) +
